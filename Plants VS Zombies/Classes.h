...
public:
	...
	//Позиция вылета projectileа
	virtual void weaponFrom() = 0;
	//Скорость полета projectileа
...
class SunflowerImage : public FlowerImage {
	...
	void setModel() {
		currentPicture = "modelSunFlower.png";
	}
...
class PeaImage : public FlowerImage {
	void weaponFrom() {
		weapon = "projectilePeashooter.png";
		//Позиция вылета projectileа на картинке
		posWeaponX = 100;
		posWeaponY = 28;
	}
	...
	void setModel() {
		currentPicture = "modelPeashooter.png";
	}
	void updateBullet(RenderWindow& window, double time) {
		posWeaponX += dx * time;
		...
		Image bulletRender;
		bulletRender.loadFromFile("projectilePeashooter.png");
		Texture texture;
...
class CabbageImage : public FlowerImage {
	...
	void setModel() {
		currentPicture = "modelCabbage.png";
	}
	void updateBullet(RenderWindow& window, double time) {
		posWeaponX += dx * time;
		//Загрузка новой анимации
		...
		Image bulletRender;
		bulletRender.loadFromFile("shellCabbage.png");
		Texture texture;
		texture.loadFromImage(bulletRender);
		Sprite newSprite;
		newSprite.setTexture(texture);
...
//Peashooter плюет горошинами (больно) 
...
//Cabbage - не промахивается кочерыжками!
...
public:
	...
	bool needToActivate(int x2) {
		//x1 - координата shurikenа
		//x2 - координата зомби
		//Если зомби правее shurikenа, то активироваться
		if (x2 > weaponX) {
			zombieInWay = true;
		}
		return zombieInWay;
...
//Фабрика SunFlowerов возвращает SunFlower и его Физику
